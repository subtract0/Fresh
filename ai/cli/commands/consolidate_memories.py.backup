...
def _normalize_text(text: str) -> str:
    # Basic normalization to enable reliable deduplication
...
def _read_text_file(path: Path) -> List[str]:
    with path.open('r', encoding='utf-8') as f:
        content = f.read()
        # Split into paragraphs (double newline) or fallback to lines
...
def _extract_text_from_obj(obj: Any) -> Optional[str]:
    ...
    if isinstance(obj, dict):
        # Common keys that may contain memory text
        for key in ("text", "content", "note", "memo", "message", "body"):
            if key in obj and isinstance(obj[key], str):
                return obj[key]
        # If small dict can be serialized
        try:
...
def consolidate_memory_logic(config: Optional[dict], verbose: bool) -> dict:
    ...
    try:
        # Determine parameters with sensible defaults
        memory_dir = Path(config.get("memory_dir")) if config and config.get("memory_dir") else Path.cwd() / "memories"
        if not memory_dir.exists():
            # If directory doesn't exist and config asked to create, create it; otherwise return no-data
        ...
        if file_patterns and isinstance(file_patterns, list) and file_patterns:
            ...
            # also include any file if none found
            if not files:
        ...
        # Optional optimizations: summarization placeholder (lightweight)
        if config and config.get("summarize", True):
            # create small excerpt for each entry if text is long
            for e in entries:
                txt = e.get("text", "")
                if len(txt) > 400:
                    e["excerpt"] = txt[:400] + "…"
...
def consolidate_memories(ctx, verbose: bool, output: str, config: Optional[str]):
    ...
    try:
        ...
        if output == 'json':
            ...
            # present a subset for readability
